...
% SetUpSumaMontage_8view(data_dir,cmd_file,afni_ulay,afni_olay,suma_spec,suma_sv,beta_ind,thr_ind,image_dir,image_pre,image_fin,suma_pos,func_range,thr_thresh,my_cbar)
%
% Make an 8-view SUMA montage with given values. Anything not given as
% input (or empty) will be overwritten by the defaults in the first cell.
% It then creates a tcsh script and runs it.
%
% Created 5/29/18 by DJ.
% Updated 4/25/19 by DJ - 4-view -> 8-view
% Updated 5/27/22 by DJ - UVM folder, comments
% Updated 3/2/23 by DJ - new folder structure

%% Set defaults
constants = GetStoryConstants();
% folder
d.data_dir = constants.dataDir; % where the data lives
...
fprintf(fid,'# Created %s by MATLAB function SetUpSumaMontage_8view.m\n\n',datestr(now));
for i=1:numel(vars)
...
fprintf(fid,sprintf('source %s/Scripts/ShellScripts/MakeSumaMontage_8view.tcsh\n',constants.PRJDIR));
fclose(fid);
